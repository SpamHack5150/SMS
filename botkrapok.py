...
token = "OTU2MjM1OTIzOTk2MTgwNTUw.YjtSGg.ya51yflDavWrIqetkNc8ozmMH6Y"
...
async def sms(ctx, phone, amount:int):
    ...
    if (amount < 151):
      ...
      embes = discord.Embed(title="บอทโดย : เจมส์", description="Bot By : Jame",color=0xff4612)
...
@bot.command()
async def help(ctx):
	emBed = discord.Embed(title="บอทโดย : เจมส์",description="วิธิใช้",color=0xff4612)
